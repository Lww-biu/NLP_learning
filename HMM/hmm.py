    ...
    for key0 in array_A:
        for key1 in array_A[key0]:
            if array_A[key0][key1] == 0.0:
                ...
                array_A[key0][key1] = log(array_A[key0][key1] /
                                          count_dic[key0])
            ...
            for state1 in STATES:
                if sentence[i] not in array_b[state0]:  # 所有在测试集出现但没有在训练集中出现的字符
                    if sentence[i - 1] not in array_b[state0]:
                        prob = tab[i - 1][state1] + array_a[state1][
                            state0] + array_b[state0]['end']
                    else:
                        prob = tab[i - 1][state1] + array_a[state1][
                            state0] + array_b[state0]['begin']
                else:
                    prob = tab[i -
                               1][state1] + array_a[state1][state0] + array_b[
...
if __name__ == '__main__':
    trainset = open('pku_training.txt', encoding='utf-8')  # 读取训练集
    testset = open('pku_test.txt', encoding='utf-8')  # 读取测试集
    # trainlist = []
